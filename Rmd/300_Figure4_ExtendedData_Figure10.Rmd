...
combined <- readRDS("Ressler2024/metadata/100_CombinedTCR.Rds")
...
order = c("Tregs", "gd T cells",   "Naive CD4+ T cells", "Exhausted T cells","Natural killer cells","Cytotoxic T cells","Memory T cells",  "T helper cells","Proliferating cells", "other")
...
```{r Fig2_bubblePlot_v1, echo=FALSE, fig.height=10, fig.width=26, message=FALSE, warning=FALSE, out.width='100%'}
...
```{r Fig3_ViolinPlot_Ratio, fig.height=6, fig.width=5, message=FALSE, warning=FALSE, out.width='100%'}
library(tidyverse)
library(tidyr)
...
df <- s@meta.data %>%
    mutate_if(sapply(s@meta.data, is.character), as.factor)  %>% 
    group_by( WHO, patient, anno_l2,    .drop = FALSE)%>% 
  summarise(Nb = n()) %>%
  mutate(C = sum(Nb)) %>%
...
DT::datatable(rbind(p1$data, p2$data, p3$data, p4$data, p5$data, p6$data, p7$data, p8$data, p9$data), 
              caption = ("Extended Data Figure 10A"),
              extensions = 'Buttons', 
              options = list( dom = 'Bfrtip',
              buttons = c( 'csv', 'excel')))
...
# Figure 4E
```{r echo=TRUE, fig.height=10, fig.width=20, message=FALSE, warning=FALSE, out.width='100%'}
colors = c("Small (5e-05 < X <= 5e-04)"   = "#8c6bb1",
...
```{r echo=TRUE, fig.height=6, fig.width=12, message=FALSE, warning=FALSE, out.width='100%'}
...
Idents(s) <- s$anno_l2
  ...
  p1 <- ggplot(tmp2, mapping = aes(x=tmp2$UMAP_1, y = tmp2$UMAP_2)) +
    ...
    scale_colour_manual(values = colors_T, breaks = levels(s$anno_l2)) +
    xlab("UMAP 1") + ylab("UMAP 2") + ggtitle(" Density of \n hyperexpanded")
  p1
  
  DT::datatable(p1$data, 
              caption = ("Extended Data Figure 10Ba"),
              extensions = 'Buttons', 
              options = list( dom = 'Bfrtip',
              buttons = c( 'csv', 'excel')))
  ...
  p2
   ...
   DT::datatable(p2$data, 
              caption = ("Extended Data Figure 10Bb"),
              extensions = 'Buttons', 
              options = list( dom = 'Bfrtip',
              buttons = c( 'csv', 'excel')))
  ...
  p3
   ...
   DT::datatable(p3$data, 
              caption = ("Extended Data Figure 10Bc"),
              extensions = 'Buttons', 
              options = list( dom = 'Bfrtip',
              buttons = c( 'csv', 'excel')))
...
p4
 ...
 DT::datatable(p4$data, 
              caption = ("Extended Data Figure 10Bd"),
              extensions = 'Buttons', 
              options = list( dom = 'Bfrtip',
              buttons = c( 'csv', 'excel')))
...
```{r Fig5_clono_quantif, echo=TRUE, fig.height=6.5, fig.width=25, message=FALSE, warning=FALSE, out.width='100%'}
...
```{r Fig5_repertoireClonalityy, echo=TRUE, fig.height=7, fig.width=35, message=FALSE, warning=FALSE, out.width='100%'}
...
DT::datatable(g1$data, 
              caption = ("Extended Data Figure 10Ca-c"),
              extensions = 'Buttons', 
              options = list( dom = 'Bfrtip',
              buttons = c( 'csv', 'excel')))

DT::datatable(g6$data, 
              caption = ("Extended Data Figure 10Cd"),
              extensions = 'Buttons', 
              options = list( dom = 'Bfrtip',
              buttons = c( 'csv', 'excel')))
```
...
```{r echo=TRUE, fig.height=8, fig.width=15, message=FALSE, warning=FALSE, out.width='100%'}
...
p <- ggplot(tmp, aes(Count, ID)) +
    geom_bar(stat = "identity", color="black", fill =  colors["Hyperexpanded (0.05 < X <= 1)"]) +
   theme_classic()+ 
    geom_text(
...
McPAS <- read.csv("Ressler2024/metadata/McPAS-TCR.csv")
head(McPAS)
...
table(s$patient, s$Category)
table(s$patient, s$Pathology)
...
```{r echo=TRUE, fig.height=10, fig.width=15, message=FALSE, warning=FALSE, out.width='100%'}
Idents(s) <- "Category"
...
colors = c("hHSV: CAVGSARQLTF_CSVWAGGTQPQHF" = "#35978f",
...
p3$labels$y <- " "
 ...
 p3 + p4 + plot_layout(ncol = 1,  heights   = c(1,2),  guides = "collect") + ylab("Number of mapped TCR")
